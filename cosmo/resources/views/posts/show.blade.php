        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="d-flex align-items-center mb-3">
                    @if ($post->user)
                        <img
                            src="{{ $post->user->profile_image ? asset('storage/' . $post->user->profile_image) : asset('user-placeholder.png') }}"
                    ...
                    @else
                        <img
                            src="{{ asset('user-placeholder.png') }}"
                            class="rounded-circle me-3"
                ...
                <div class="d-flex align-items-center mb-2">
                    @if ($post->likes->where('user_id', auth()->id())->count() > 0)
                        <form
                            action="{{ route('likes.destroy', $post->id) }}"
                            method="POST"
                            class="me-2"
                        >
                            @csrf
                    ...
                    @else
                        <form
                            action="{{ route('likes.store', $post->id) }}"
                            method="POST"
                            class="me-2"
                        >
                            @csrf
                ...
                <div class="comments-section" style="max-height: 300px; overflow-y: auto">
                    @foreach ($post->comments as $comment)
                        <div class="d-flex mb-2">
                            @if ($comment->user)
                                <img
                                    src="{{ $comment->user->profile_image ? asset('storage/' . $comment->user->profile_image) : asset('user-placeholder.png') }}"
                                    class="rounded-circle me-2"
                                    style="width: 32px; height: 32px; object-fit: cover"
                                />
                                <strong class="me-2 text-white">
