                ...
                <div class="card bg-dark text-white border-secondary">
                    <div class="card-header bg-secondary d-flex align-items-center">
                        @if($post->user)
                            ...
                            <a href="{{ route('profile.show', $post->user->id) }}" class="text-white text-decoration-none">
                                <strong>{{ $post->user->username }}</strong>
                            </a>
                        @else
                            <img src="https://via.placeholder.com/40" class="rounded-circle me-3" style="width: 40px; height: 40px;">
                            <span class="text-light">Deleted User</span>
                        @endif
                    </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex mb-2">
                            ...
                            @else
                                <form action="{{ route('likes.store', $post->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-link text-white p-0 me-2">
                                        <i class="far fa-heart"></i>
                                    </button>
                                </form>
                            @endif
                            <a href="{{ route('posts.show', $post->id) }}" class="btn btn-link text-white p-0">
                        ...
                        <p class="text-secondary mt-1">{{ $post->created_at->diffForHumans() }}</p>
                    </div>
                </div>
            </div>
        @endforeach
